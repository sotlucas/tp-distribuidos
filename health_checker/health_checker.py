...
class HealthCheckerConfig:
    def __init__(self, replica_id, filter_general_replicas, filter_multiple_replicas,
...
class HealthChecker:
    ...
    def init_checker_hc(self, processor_prefix, current_replica_id, processor_checkers, processor_suffix="-1"):
        """
    ...
    def connect_to_processor(self, processor_name):
        ...
        while self.running:
            try:
                ...
                logging.exception(f"Error: {e}")
                self.restart_container(processor_name)
                logging.warning(f"Connection failed to processor {processor_name}. Retrying...")
                time.sleep(HEALTH_CHECK_INTERVAL)
        return buff
